"""
    @file:              BreastCancerExperiment.py
    @Author:            Alexandre Ayotte
                        Nicolas Raymond
    @Creation Date:     07/12/2019
    @Last modification: 07/12/2019
    @Description:       For this experiment, we will evaluate the performance of all hyper-parameter optimization
                        ...
                        considering breast cancer wisconsin data set classification problem (569 instances,
                        30 attributes, 2 classes), will we initialize a Sklearn MLP with 4 hidden layers of 20 neurons
                        with default parameter and try to find the best values for alpha (L2 penalty (regularization
                        term) parameter), learning rate init (initial learning rate used), beta1  (exponential decay
                        rate for estimates of first moment vector in adam), beta2 (exponential decay rate for estimates
                        of second moment vector in adam), hidden layers number (between 1 and 20) and finally layers
                        size (number of neurons on each hidden layer between 20 and 100) with all methods available.
...
x_train, t_train, x_test, t_test = dm.load_breast_cancer_dataset(random_state=42)
...
gs_tuner.set_search_space({'alpha': DiscreteDomain(list(linspace(10 ** -8, 1, 5))),
                           ...
                           'layers_size': DiscreteDomain([40, 80])})
